...
router.get("/financial", async (req, res) => {
  ...
  if (!shifts.length) {
    ...
    const financialData = shifts.reduce(
      (data, shift) => {
        let floatTotalDriversHours = 0;
        data.profit = data.profit || 0;
        data.profit += parseFloat(
          (shift.officeValue - shift.driverValue).toFixed(2)
        );
        ...
        data.driversTotalHours += toFloat(
          shift.totalWorkingHours,
          shift.totalWorkingMinutes
        );
        return data;
      },
      { id: "financial" }
    );
    financialData.driversTotalHours = toHoursAndMinutes(
      financialData.driversTotalHours
    );
...
router.get("/top3drivers", async (req, res) => {
  ...
  const topDrivers = Array.from(driverAndValue).sort((a, b) => b[1] - a[1]);
...
router.get("/top3workplaces", async (req, res) => {
  ...
  const workplacesAndValues = shifts
    .map((shift) => {
      return {
  ...
  const topWorkplaces = Array.from(workplacesAndValues).sort(
    (a, b) => b[1] - a[1]
  );
