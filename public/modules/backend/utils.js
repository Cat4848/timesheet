...
async function isAuthorized(req, res, next) {
    ...
    jwt.verify(token, process.env.ACCESS_TOKEN_SECRET, async (err, user) => {
        if(err) {
            console.log("is Authorized function, the if statement error");
            if(err.message === "jwt expired") {
                ...
                //to cookies
                console.log("is Authorized function -> generate new access token");
                jwt.verify(req.session.refreshToken, process.env.REFRESH_TOKEN_SECRET, ((error, user) => {
                  if (error) {
                    req.logout(error => {
